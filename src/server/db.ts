...
export async function seed() {
  ...
  const insertUser = db.prepare("INSERT INTO users (username, display_name, password_hash, avatar_color) VALUES (?, ?, ?, ?)");
  ...
  const appRows = [
    ...
    ["messenger", "Messenger", "Msg", "Live chat with friends in desktop windows.", "Social"],
  ...
  const insertApp = db.prepare("INSERT INTO apps (id, name, icon, description, category) VALUES (?, ?, ?, ?, ?)");
  for (const app of appRows) insertApp.run(...app);

  const install = db.prepare("INSERT INTO installed_apps (user_id, app_id) VALUES (?, ?)");
  for (const app of ["gopost", "settings", "minecraft", "messenger", "browser"]) install.run(1, app);

  db.prepare("INSERT INTO user_settings VALUES (?, ?, ?, ?, ?)").run(1, "dev-bright", "glass", 1, 0);
  db.prepare("INSERT INTO user_settings VALUES (?, ?, ?, ?, ?)").run(2, "dev-bright", "glass", 1, 1);
  db.prepare("INSERT INTO user_settings VALUES (?, ?, ?, ?, ?)").run(3, "dev-bright", "glass", 1, 0);
  ...
  const insertPost = db.prepare("INSERT INTO posts (author_id, body, image_style) VALUES (?, ?, ?)");
  ...
  const insertMessage = db.prepare("INSERT INTO messages (conversation_id, sender_id, body) VALUES (?, ?, ?)");

...
import { db, desktopState, migratePlatform, seedPlatform, settings } from "./repo";
...
const routes: Route[] = [
  ...
  ["POST", /^\/apps\/([^/]+)\/install$/, ["id"], ({ params, userId }) => {
    ...
    db.prepare("UPDATE desktop_state SET dock_apps = ? WHERE user_id = ?").run(JSON.stringify(state.dockApps), userId);
    return json({ ok: true });
  }],
  ["GET", /^\/settings$/, [], ({ userId }) => {
    const unauthorized = requireUser(userId);
  ...
  ["PATCH", /^\/settings$/, [], async ({ request, userId }) => {
    const unauthorized = requireUser(userId);
    if (unauthorized) return unauthorized;
    ...
    `).run(userId, next.wallpaper, next.dockStyle, next.notifications ? 1 : 0, next.classicSounds ? 1 : 0);
    return json({ settings: settings(userId!) });
  }],
  ["GET", /^\/desktop\/state$/, [], ({ userId }) => {
    const unauthorized = requireUser(userId);
    if (unauthorized) return unauthorized;
    return json({ desktopState: desktopState(userId!) });
  }],
  ["PATCH", /^\/desktop\/state$/, [], async ({ request, userId }) => {
    const unauthorized = requireUser(userId);
    if (unauthorized) return unauthorized;
    ...
    `).run(userId, JSON.stringify(next.dockApps), JSON.stringify(next.openedApps), next.wallpaper);
    return json({ desktopState: desktopState(userId!) });
  }],
  ["GET", /^\/minecraft\/profile$/, [], ({ userId }) => {
    const unauthorized = requireUser(userId);
    if (unauthorized) return unauthorized;

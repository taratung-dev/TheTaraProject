...
export function seedSocial() {
  ...
  const postCount = db.query("SELECT COUNT(*) AS count FROM posts").get() as {
    count: number;
  };
...
export function postRows(
  ...
  `,
      ...
      .all(userId ?? 0, ...params) as PostJoinRow[]
  ).map((row) => ({
    id: row.id,
    body: row.body,

...
import { createSession, createUser, deleteSession, migrateAuth, newToken, seedAuth, sessionUserId, userById, userByUsername } from "./repo";
...
const routes: Route[] = [
  ...
  ["GET", /^\/internal\/session$/, [], ({ request }) => {
    const userId = sessionUserId(parseCookie(request).session);
    return json({ userId });
  }],
  ["POST", /^\/auth\/signup$/, [], async ({ request }) => {
    ...
    try {
      const userId = createUser(input.username, input.displayName, await Bun.password.hash(input.password));
      const token = newToken();
      createSession(userId, token);
      return json({ user: userById(userId) }, 201, { "Set-Cookie": sessionCookie(token) });
    } catch {
  ...
  ["POST", /^\/auth\/login$/, [], async ({ request }) => {
    ...
    return json({ user: userById(row.id) }, 200, { "Set-Cookie": sessionCookie(token) });
  }],
